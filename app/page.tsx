...
export default function Dashboard() {
  ...
  const fetchSettings = useCallback(async () => { const { data } = await supabase.from('user_settings').select('*').single(); if (data) { setBankName(data.bank_name); setShowBankBreakdown(data.show_bank_breakdown); setCcBillingDay(data.cc_billing_day || 15); setCcDueDay(data.cc_due_day || 5); } else if (session) { await supabase.from('user_settings').insert([{ user_id: session.user.id, bank_name: 'IDFC Account Breakdown', show_bank_breakdown: false, cc_billing_day: 15, cc_due_day: 5 }]); setBankName('IDFC Account Breakdown'); } }, [session]);
  const fetchData = useCallback(async (month: string) => { setDataLoading(true); setLedgerData([]); const [{ data: monthly }, { data: future }] = await Promise.all([supabase.from('ledger').select('*').eq('month', month).neq('type', 'Future Purchases'), supabase.from('ledger').select('*').eq('type', 'Future Purchases')]); const monthEntries = (monthly ?? []) as LedgerEntry[]; const existingSummaryCategories = new Set(monthEntries.filter((entry) => entry.type === 'Summary').map((entry) => entry.category)); const missingSummaryEntries = ['Investments', 'Savings'].filter((category) => !existingSummaryCategories.has(category)).map((category) => ({ month, category, amount: 0, type: 'Summary', user_id: session?.user.id })); const { data: createdSummaries } = missingSummaryEntries.length ? await supabase.from('ledger').insert(missingSummaryEntries).select() : { data: [] }; setLedgerData([...monthEntries, ...((createdSummaries ?? []) as LedgerEntry[]), ...((future ?? []) as LedgerEntry[])]); setDataLoading(false); }, [session]);
  const fetchOverallStats = useCallback(async () => { const { data } = await supabase.from('ledger').select('category, amount').eq('type', 'Summary'); if (data) setOverallStats({ investments: data.filter((item) => item.category === 'Investments').reduce((sum, item) => sum + Number(item.amount), 0), savings: data.filter((item) => item.category === 'Savings').reduce((sum, item) => sum + Number(item.amount), 0) }); }, []);
